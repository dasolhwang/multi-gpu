...
class GPUNetworkBuilder(object):
    ...
    def dwconv(self, input_layer, filter_size, filter_strides=(1, 1),
               padding='SAME', channel_multiplier=1.0, activation=None,
               use_batch_norm=None):
        ...
        with tf.variable_scope(self._count_layer('dwconv')) as scope:
            kernel = self._get_variable('weights', kernel_shape,
                                        input_layer.dtype)
            x = tf.nn.depthwise_conv2d(input_layer, kernel, strides,
                                       padding, data_format='NCHW')
            x = self._bias_or_batch_norm(x, scope, use_batch_norm)
            x = self.activate(x, activation)
            return x

    def squeeze_excitation(self, input_layer, reduction_ratio=16):
        num_filters = input_layer.get_shape().as_list()[1]
        with tf.variable_scope(self._count_layer('squeeze_excitation')):
            x = tf.reduce_mean(input_layer, [2, 3], True)
            x = tf.reshape(x, [64, -1])
            x = self.fully_connected(x, num_filters // reduction_ratio,
                                     'RELU')
            x = self.fully_connected(x, num_filters, 'SIGMOID')
            x = tf.reshape(x, [-1, num_filters, 1, 1])
            scale = input_layer * x
            return scale

    def gconv2d(self, input_layer, filters, filter_strides=(2, 2),
                padding='SAME', num_groups=None, dilation_rate=None):

        input_list = tf.split(input_layer, num_groups, axis=1)
    ...
    def gconv(self, input_layer, num_filters, filter_size,
              filter_strides=(1, 1), padding='SAME', num_groups=None,
              dilation_rate=None, activation=None, use_batch_norm=None):
        if num_groups == 1:
            x = self.conv(input_layer, num_filters, filter_size,
                          filter_strides, padding, activation,
                          use_batch_norm)
            return x
        if num_groups < 1:
            raise ValueError("Specify the number of groups greater than zero")
        num_inputs = input_layer.get_shape().as_list()[1]
        with tf.variable_scope(self._count_layer('gconv')) as scope:
            kernel_shape = list(filter_size) + [num_inputs / num_groups] + [num_filters]
            kernels = self._get_variable('weights', kernel_shape, input_layer.dtype)
            x = self.gconv2d(input_layer, kernels, filter_strides,
                             padding, num_groups, dilation_rate)
            x = self._bias_or_batch_norm(x, scope, use_batch_norm)
            x = self.activate(x, activation)
            return x
...
def inference_antnet(net, input_layer):
    ant_defs = [ # upsample, channel, strides, repeat, group, ratio]
         [(1, 16, (1,1), 1, 1, 8)],  # second block, input : 112x112x32
         [(6, 24, (2,2), 2, 2, 8)],  # third block, input: 112x112x16
         [(6, 32, (2,2), 3, 2, 12)],  # fourth block, input: 56x56x24
         [(6, 64, (2,2), 4, 2, 16)],  # fifth block, input: 28x28x32
         [(6, 96, (1,1), 3, 2, 24)],  # sixth block, input: 28x28x64
         [(6, 160, (2,2), 3, 2, 32)],  # seventh block, input: 14x14x96
         [(6, 320, (1,1), 1, 2, 64)]  # eighth block, input: 7x7x160
    ]
...
def main():
    ...
    cmdline.add_argument('--num_batches', default=int(1281167/64), type=int,
...
#        nrecord = FLAGS.num_batches * total_batch_size
    ...
    elif model_name == 'antnet':
        height, width = 224, 224
        model_func = inference_antnet
